...
std::uint8_t* Student::getIdeaChecksum() {
    return sha256(currentIdea->getName());
}

std::uint8_t* Student::getPackagesChecksum() {
...
void Student::buildIdea() {
    ...
    // Present the idea
    {
        std::uint8_t* ideaChecksum = getIdeaChecksum();
        ChecksumTracker<Student, ChecksumType::IDEA>::updateGlobalChecksum(ideaChecksum);

        std::uint8_t* packagesChecksum = getPackagesChecksum();
