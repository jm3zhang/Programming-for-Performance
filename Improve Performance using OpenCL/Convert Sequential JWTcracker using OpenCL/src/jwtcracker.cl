...
__kernel void bruteForceJWT(global const uchar *gAlphabet, global const uchar *message, global const uchar *origSig, global uchar *secret)
{   
    // get the current id 
    size_t id = get_global_id(0);

    // use this id to assign different jwt to crack
    uchar base36_secret[gMaxSecretLen];

    // convert id to base alphabetLenght and put into the array
    int index = 0;
    int digit = 0;

    // padding 
    for (int i = 0; i < gMaxSecretLen; i++){
        base36_secret[i] = gAlphabet[0];
    }

    if (id == 0){
        base36_secret[index] = gAlphabet[0];
        index ++;
    }

    while (id > 0){
        digit = id % alphabetLenght;
        // 36^0, 36^1, 36^2 ...
        base36_secret[index] = gAlphabet[digit];
        id /= alphabetLenght;
        index ++;
    }
    ...
    uint8_t uint8_base36_secret[gMaxSecretLen];
    for (int i = 0; i < gMaxSecretLen; i++){
        uint8_base36_secret[i] = (uint8_t) base36_secret[i];
    }

    // convert message to uint8_t
    uint8_t uint8_message[messageLenght];
    for (int i = 0; i < messageLenght; i++){
        uint8_message[i] = (uint8_t) message[i];
    }

    // check the padding case when the length of the secret is not gMaxSecretLen
    for (int i = index; i < gMaxSecretLen + 1; i ++){
        uint8_t sigBuffer[sigLen];
        uint32_t messageBuffer[messageLenght + BLOCK_BYTES];
        
        // if(uint8_base36_secret[3] == 'a' && uint8_base36_secret[2] == '3' && uint8_base36_secret[1] == '2' && uint8_base36_secret[0] == '1'){
        //     printf("YES");
        // }
        
        HMAC_SHA256(uint8_base36_secret, gMaxSecretLen, uint8_message, messageLenght, messageBuffer, sigBuffer);

        // now verify if the secret is right
        int output = 1;
        for (int i = 0; i < HASH_BYTES; i++){
        ...
        if (output) {
            for (int i = 0; i < gMaxSecretLen; i++) {
                secret[i] = uint8_base36_secret[i];
            }
        }
    }
}
