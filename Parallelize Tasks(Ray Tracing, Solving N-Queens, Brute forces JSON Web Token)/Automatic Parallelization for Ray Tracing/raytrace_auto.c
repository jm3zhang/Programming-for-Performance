/* A simple ray tracer */
/* Source: http://www.purplealienplanet.com/node/23 */
/* Author writes: "The code is licensed under BSD, so use it however you want". */
...
bool intersectRaySphere(ray *r, sphere *s, float *t){
	...
	float A = (float)VECTORDOT(r->dir, r->dir); 
	
	/* We need a vector representing the distance between the start of 
	 * the ray and the position of the circle.
	 * This is the term (p0 - c) 
	 */
	vector dist = (vector)VECTORSUB(r->start, s->pos);
	
	/* 2d.(p0 - c) */  
	float B = 2 * (float)VECTORDOT(r->dir, dist);
	
	/* (p0 - c).(p0 - c) - r^2 */
	float C = (float)VECTORDOT(dist, dist) - (s->radius * s->radius);
...
int main(int argc, char *argv[]){
	...
	unsigned char * img = malloc(3*WIDTH*HEIGHT*sizeof(unsigned char));
	...
	for(y=0;y<HEIGHT;y++){
		for(x=0;x<WIDTH;x++){
			...
			do{
				...
				vector scaled = (vector)VECTORSCALE(t, r.dir);
				vector newStart = (vector)VECTORADD(r.start, scaled);
				
				/* Find the normal for this new vector at the point of intersection */
				// vector n = vectorSub(&newStart, &spheres[currentSphere].pos);
				vector n = VECTORSUB(newStart, spheres[currentSphere].pos);
				float temp = (float)VECTORDOT(n, n);
				if(temp == 0) break;
				
				temp = 1.0f / sqrtf(temp);
				n = (vector)VECTORSCALE(temp, n);
				...
				for(j=0; j < 3; j++){
					...
					vector dist = VECTORSUB(currentLight.pos, newStart);
					if((float)VECTORDOT(n, dist) <= 0.0f) continue;
					float t = sqrtf((float)VECTORDOT(dist,dist));
					if(t <= 0.0f) continue;
					
					ray lightRay;
					lightRay.start = newStart;
					lightRay.dir = (vector)VECTORSCALE((1/t), dist);
					
					/* Lambert diffusion */
					float lambert = (float)VECTORDOT(lightRay.dir, n) * coef; 
				...
				r.start = newStart;
				float reflect = 2.0f * (float)VECTORDOT(r.dir, n);
				vector tmp = (vector)VECTORSCALE(reflect, n);
				// r.dir = vectorSub(&r.dir, &tmp);
				r.dir = (vector)VECTORSUB(r.dir, tmp);
	...
	saveppm("image_auto.ppm", img, WIDTH, HEIGHT);
